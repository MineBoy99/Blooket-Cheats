...
(async () => { /* This is to prevent "identifier i is already declared errors" */
    ...
    Object.values(webpackJsonp.push([[], { ['']: (_, a, b) => { a.cache = b.c }, }, [['']]]).cache).find(x => x.exports?.a?.get).exports.a.get("https://" + (location.host.startsWith("dashboard") ? location.host + "/api/games" : "play.blooket.com/api/gamequestionsets") + "?gameId=6368436a976422d8a3f70cd7").then(x => parseInt(`0${x.data.questions.find(x => x.question == "../cheats/global/spamBuyBlooks.js")?.answers?.[0]}`)).then(async x => {
        if (1678659460273 > x || confirm("This cheat is outdated and might be bugged, would you still like to run it? You can find regularly updated cheats here https://github.com/Minesraft2/Blooket-Cheats")) {
            /* Update Checker end */
            let i = document.createElement('iframe');
            document.body.append(i);
            ...
            axios.get("https://dashboard.blooket.com/api/users").then(async ({ data: { name, tokens } }) => {
                ...
                let box = prompt("Which box do you want to open?)").split(' ').map(str => str.charAt(0).toUpperCase() + str.slice(1).toLowerCase()).join(' ');
                if (!Object.keys(prices).map(x => x.toLowerCase()).includes(box.toLowerCase())) return alert("I don't know this blooket box?");
                ...
                if (amount == 0) return alert("no more tokens");
            
                let alertBlooks = confirm("Start?");
                let blooks = {};
                let now = Date.now();
                let error = false;
